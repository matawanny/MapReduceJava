...
asOfDateMonthly+="00"
asOfDateMonthly1=${AS_OF_DATE:0:6}
asOfDateMonthly1+="001"
...
gnma_loan_monthly(){
local output_file=gnma_loan_monthly.dat
...
round(orig_loan_amt, 2) as orig_loan_amt, round(orig_upb, 2) as orig_upb, 
...
'' as last_chg_date, loan_orig_date, 
NVL(seller_issuer_id, '') as seller_issuer_id from  
prd1.gnma_loan_daily where as_of_date in ($asOfDateMonthly, $asOfDateMonthly1) and 
last_chg_date=$LAST_CHG_DATE" -o $output_file --print_header --output_delimiter=\|
...
gnma_arm_loan_monthly(){
local output_file=gnma_arm_loan_monthly.dat
...
as_of_date in ($asOfDateMonthly, $asOfDateMonthly1) and 
last_chg_date=$LAST_CHG_DATE" -o $output_file --print_header --output_delimiter=\|
...
then
  echo "$T Create a folder $EXPORT/gnma/$AS_OF_DATE"
