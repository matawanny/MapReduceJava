...
ls $EXPORT/fnma/$lastBusinessDay/*loan_daily.SIG >/dev/null 2>&1
if [[ $? -ne 0 ]]
then
  d=$lastBusinessDay
else
...
do
        ...
        then
          ...
          then
	          ...
	          sh $APP/yb-apache-hbase/src/main/scripts/fnma/Impala_fnma_daily_export.sh $as_of_date $daily_return_code
	      fi
